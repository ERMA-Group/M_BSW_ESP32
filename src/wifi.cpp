...
esp_err_t Wifi::root_get_handler(httpd_req_t* req)
{
    ...
    html += "<label>Operating Mode</label><select id='operating_mode' name='operating_mode'><option value='cloud_ha' selected>Cloud+HA (default)</option><option value='home_assistant'>Home Assistant</option></select>";
    ...
    html += "<p class='hint'>Wi-Fi SSID and password are required in both modes. Cloud+HA mode enables cloud pairing/sync and Home Assistant MQTT bridge. Home Assistant mode disables cloud and runs Home Assistant MQTT only.</p>";
    html += "<script>(function(){var mode=document.getElementById('operating_mode');var pin=document.getElementById('pin_fields');var pairingPin=document.getElementById('pairing_pin_input');function update(){var mqtt=(mode.value==='home_assistant');pin.style.display=mqtt?'none':'block';if(pairingPin){pairingPin.required=!mqtt;}}mode.addEventListener('change',update);update();})();</script>";
    html += "</div></body></html>";
...
esp_err_t Wifi::save_post_handler(httpd_req_t* req)
{
    ...
    const std::string pairing_pin = get_form_value(body, "pairing_pin");
    const bool pure_mqtt = (operating_mode == "home_assistant" || operating_mode == "pure_mqtt");
    if (ssid.empty())
    {
    ...
    const char* response = "Saved. Device will restart and apply selected mode.";
