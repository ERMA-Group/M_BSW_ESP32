...
void Wifi::set_provisioning_html_callback(const std::function<std::string(const std::string& current_ssid, const std::string& current_password)>& callback)
{
    provisioning_html_cb_ = callback;
}
...
esp_err_t Wifi::root_get_handler(httpd_req_t* req)
{
    ...
    std::string html;
    if (self->provisioning_html_cb_)
    {
        html = self->provisioning_html_cb_(self->config_.wifi_ssid, self->config_.wifi_password);
    }
    else
    {
        html = "<!doctype html><html><head><title>Provisioning</title></head><body><h2>No HTML Provided</h2></body></html>";
    }
