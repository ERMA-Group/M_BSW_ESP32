...
esp_err_t Wifi::root_get_handler(httpd_req_t* req)
{
    ...
    html += "<label>Pairing PIN</label><input name='pairing_pin' required>";
    html += "<button type='submit'>Save and Restart</button></form>";
    html += "<p class='hint'>Use the pairing PIN shown in the web application.</p></div></body></html>";
...
esp_err_t Wifi::save_post_handler(httpd_req_t* req)
{
    ...
    {
        self->pairing_pin_callback_(pairing_pin);
    }

    self->portal_submitted_ = true;
    const char* response = "Saved. Device will restart, connect to configured Wi-Fi, and pair.";
